...
export default async function CategoryPage(props: { params: { slug: string }, searchParams: { page?: string } }) {
  ...
  const { data: posts, count } = await getPosts(page, { status: 'published', categoryId: category.id });
    ...
    <main className="flex-1 w-full bg-slate-50 dark:bg-slate-950/50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
        <header className="mb-16 md:mb-24 max-w-3xl text-center mx-auto">
          <span className="text-blue-600 font-bold uppercase tracking-wider text-sm mb-4 block">Category</span>
        ...
        {posts.length === 0 && (
          <p className="text-slate-500 dark:text-slate-400 py-12 text-center text-lg">No posts found in this category.</p>
        )}
