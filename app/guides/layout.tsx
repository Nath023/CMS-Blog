    ...
    <div className="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 min-h-screen flex flex-col font-sans transition-colors duration-300">
      ...
      <nav className="h-20 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 md:px-8 flex items-center justify-between shrink-0 sticky top-0 z-50 transition-colors duration-300 shadow-sm">
        <Link href="/" className="flex items-center gap-3 group">
          <Image src={siteLogoUrl} alt={settings?.site_name || "Site Logo"} width={800} height={800} quality={100} className="w-auto h-12 object-contain" referrerPolicy="no-referrer" priority />
        ...
        <div className="flex items-center gap-6 md:gap-8">
          ...
          <a href="https://example.com" target="_blank" rel="noopener noreferrer" className="text-sm font-semibold bg-slate-900 text-white dark:bg-white dark:text-slate-900 px-5 py-2.5 rounded-full hover:bg-primary dark:hover:bg-primary hover:text-white transition-all shadow-sm">
            Main Site &rarr;
          </a>
        </div>
      </nav>
